...
const SELECTOR = Object.values(FX)
 ...
 * Run the fracture.
 *
 * `strength` is 0–1: the echo bounce a fraction of a second after the first
 * strike runs the same animation at about a quarter. `hot` is the claim's
 * status — a paid claim lands at full intensity, a pending one at 0.66×, so the
 * two states feel different without needing a second animation.
 */
export function playImpact(root: HTMLElement, strength: number, hot: boolean) {
  if (prefersReducedMotion() || !root.animate) return;

  // A rapid re-trigger must not stack; cancel first so every element is back
  // at its static style before the new cycle starts.
  for (const node of root.querySelectorAll<HTMLElement>(SELECTOR)) {
    node.getAnimations?.().forEach((animation) => animation.cancel());
  }

  const f = strength || 1;
  const heat = (hot ? 1 : 0.66) * f;
  ...
  query(root, FX.crack).forEach((crack, i) => {
    ...
    crack.animate(
      [
        ...
        { transform: `${rotate}scaleX(1)`, opacity: (hot ? 0.4 : 0.28) * f, offset: 0.14, easing: "linear" },
    ...
    hotCore?.animate(
      [
        { opacity: 0, offset: 0 },
        { opacity: (hot ? 0.5 : 0.32) * f, offset: 0.1, easing: "cubic-bezier(.3,0,.6,.4)" },
        { opacity: 0.1 * f, offset: 0.32, easing: "ease-out" },
        { opacity: 0, offset: 1 },
      ],
  ...
  query(root, FX.chip).forEach((chip, i) => {
    ...
    const distance = (Number(chip.dataset.distance) || 60) * (0.7 + 0.3 * f);
  ...
  query(root, FX.ring).forEach((ring, i) => {
    ring.animate(
      [
        ...
        {
          transform: `translate(-50%,-50%) scale(${(1.35 + i * 0.52) * (0.8 + 0.2 * f)})`,
          opacity: 0,
        },
      ],

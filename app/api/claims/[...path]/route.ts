...
/** The member's own claims and receipt uploads. */
async function forward(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  try {
    ...
    const result = await authenticatedRequest(
      request,
      `/claims/${suffix}${request.nextUrl.search}`,
      {
        method,
        headers: contentType ? { "content-type": contentType } : undefined,
        // Bytes: a receipt upload is multipart, and decoding it to text would
        // corrupt every image that is not valid UTF-8.
